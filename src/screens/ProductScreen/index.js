  ...
  return (
    <Card sx={{ mt: -10, borderRadius: 4 }}>
      ...
      <CardContent sx={{ py: 0, px: { xs: 1, sm: 1, md: 2 } }}>
        ...
        <Typography variant="h5">محصولات این پکیج</Typography>
        <Stack
        //   bgcolor="#eee"
          columnGap={1}
          p={1}
        ...
        >
          {packaged_items &&
            ...
            packaged_items.map((packaged, i) => {
              const { id, title, brand, images, introtext } =
                packaged.product_shop?.product_info;
              const productTitle = `${title} ${brand?.title || ""}`;
              return (
                <Card variant='outlined' key={i} sx={{ width: 300 }}>
                  <CardActionArea
                  color='warning'
                    href={`/shops/${packaged.product_shop?.shop?.id}/${id}`}
                  >
                    ...
                    />
                    <CardContent>
                      {/* itemId={packaged.product_shop?.id}
                    shop={packaged.product_shop?.shop}
                    co_price={packaged.product_shop?.co_price}
                    me_price={packaged?.me_price}
                    attributes={packaged.product_shop?.attributes}
                    product_info={packaged.product_shop?.product_info}
                    discount_percent={packaged.product_shop?.discount_percent} */}
                    </CardContent>
                  </CardActionArea>
                </Card>
              );
